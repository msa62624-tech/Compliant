...
export class ProgramsController {
  ...
  @ApiQuery({ name: 'isTemplate', required: false, type: Boolean, description: 'Filter by template status' })
  @ApiResponse({ status: 200, description: 'Programs retrieved successfully' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  findAll(@Query('isTemplate') isTemplate?: string) {
    const isTemplateBoolean = isTemplate === 'true' ? true : isTemplate === 'false' ? false : undefined;
  ...
  @ApiResponse({ status: 200, description: 'Project programs retrieved successfully' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  ...
  @ApiResponse({ status: 400, description: 'Cannot delete program with active assignments' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  ...
  @ApiResponse({ status: 201, description: 'Program assigned to project successfully' })
  @ApiResponse({ status: 400, description: 'Program already assigned to project' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @ApiResponse({ status: 403, description: 'Forbidden - Admin or Super Admin role required' })
